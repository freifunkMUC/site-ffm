...
{%- for pkg in pkgs %}
    -{{ pkg }}{% if not loop.last %} \\{% endif %}
{%- endfor %}"""
        ).render(name=self.name, pkgs=self.pkgs)
...
class Target:
    ...
    def include(self, devices: [str], pkglists: [PackageList] = None):
        for device in devices:
            assert device in self.devices, "Device %s not in target %s" % (
                device,
                self.name,
            )
            if not pkglists:
...
PKGS_USB_HID = PackageList("USB_HID", ["kmod-usb-hid", "kmod-hid-generic"])
pkglists.append(PKGS_USB_HID)

PKGS_USB_SERIAL = PackageList(
    "USB_SERIAL", ["kmod-usb-serial", "kmod-usb-serial-ftdi", "kmod-usb-serial-pl2303"]
)
pkglists.append(PKGS_USB_SERIAL)

PKGS_USB_STORAGE = PackageList(
    "USB_STORAGE",
    [
        "block-mount",
        "blkid",
...
PKGS_PCI_NET = PackageList(
    "PCI_NET", ["kmod-bnx2"]  # Broadcom NetExtreme BCM5706/5708/5709/5716
)
...
PKGS_TLS = PackageList("TLS", ["ca-bundle", "libustream-openssl"])
pkglists.append(PKGS_TLS)

PKGS_NSM = PackageList(
    "NSM",
    [
...
PKGS_OPEN_VM_TOOLS = PackageList("OPEN_VM_TOOLS", ["open-vm-tools"])
pkglists.append(PKGS_OPEN_VM_TOOLS)

#
# package assignment
#

targets["ath79-generic"].add_pkglist(PKGS_TLS).include(
...
for target in [
    ...
    targets[target].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
        PKGS_USB_SERIAL
    ).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_TLS)

targets["lantiq-xrx200"].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
    PKGS_USB_SERIAL
).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_TLS).exclude(
    [  # 7M usable firmware space + USB port
        "avm-fritz-box-7412",
        "tp-link-td-w8970",
        "tp-link-td-w8980",
...
for target in [
    ...
    targets[target].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
        PKGS_USB_SERIAL
    ).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_USB_HID).add_pkglist(PKGS_TLS)

targets["mediatek-mt7622"].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
    PKGS_USB_SERIAL
).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_TLS).exclude(
    ["ubiquiti-unifi-6-lr-v1"],  # devices without usb ports
    pkglists=[PKGS_USB, PKGS_USB_NET, PKGS_USB_SERIAL, PKGS_USB_STORAGE],
)

targets["ramips-mt7621"].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
    PKGS_USB_SERIAL
).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_TLS).include(
    ["zyxel-nwa55axe"], pkglists=[PKGS_NSM]
).exclude(
...
targets["ramips-mt7620"].add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
    PKGS_USB_SERIAL
).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_TLS).exclude(
    ["netgear-ex3700"],  # devices without usb ports
    pkglists=[PKGS_USB, PKGS_USB_NET, PKGS_USB_SERIAL, PKGS_USB_STORAGE],
)

targets["ramips-mt76x8"].add_pkglist(PKGS_TLS).include(
    [  # 7M usable firmware space + USB port
        "gl-mt300n-v2",
    ...
    pkglists=[PKGS_USB, PKGS_USB_NET, PKGS_USB_SERIAL, PKGS_USB_STORAGE],
).exclude([], pkglists=[PKGS_TLS])

for target in ["x86-64", "x86-generic", "x86-geode"]:
    targets.get(target).add_pkglist(PKGS_USB).add_pkglist(PKGS_USB_NET).add_pkglist(
        PKGS_USB_SERIAL
    ).add_pkglist(PKGS_USB_STORAGE).add_pkglist(PKGS_PCI).add_pkglist(
        PKGS_PCI_NET
    ).add_pkglist(
        PKGS_TLS
    ).add_pkglist(
